...
YOUR_DEPLOYED_API_BASE_URL = "https://i-ts-golu-cp-api.vercel.app"
...
AUTH_USER = os.environ.get('AUTH_USERS', '1403488629').split(',')
...
def sanitize_filename(filename):
    """Remove or replace special characters that are problematic for file systems and ffmpeg"""
...
async def text_to_txt(client, message: Message):
    ...
    if raw_textn == '/d':
        ...
        custom_file_name = sanitize_filename(raw_textn)
...
async def youtube_to_txt(client, message: Message):
    ...
    txt_file = os.path.join("downloads", f'{sanitize_filename(title)}.txt')
    os.makedirs(os.path.dirname(txt_file), exist_ok=True)
...
async def txt_handler(bot: Client, m: Message):
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            name = sanitize_filename(f'{name1[:60]} {CREDIT}')
...
async def yt2m_handler(bot: Client, m: Message):
    ...
    try:
        ...
        name = sanitize_filename(f'{audio_title[:60]} {CREDIT}')
...
async def txt_handler(bot: Client, m: Message):  
    ...
    if raw_text0 == '/d':
        b_name = sanitize_filename(file_name.replace('_', ' '))
    else:
        b_name = sanitize_filename(raw_text0)
    ...
    if raw_text3 == '/d':
        ...
        CR = sanitize_filename(raw_text3)
    ...
    try:
        ...
        if raw_text4 == '/d':
            prog_msg = await m.reply_text("**🔄 Fetching a new Classplus token from your deployed API...**")
            try:
                async with aiohttp.ClientSession() as session:
                    api_gen_token_url = f"{YOUR_DEPLOYED_API_BASE_URL}/api/generate-token"
                    async with session.post(api_gen_token_url) as resp:
                        if resp.status == 200:
                            data = await resp.json()
                            if data.get("success"):
                                fetched_token = data.get("token")
                                if fetched_token:
                                    cptoken = fetched_token
                                    pwtoken = fetched_token
                                    cwtoken = 'eyJhbGc...cw_token...'
                                    await prog_msg.edit_text(f"✅ **New token fetched successfully from your API!**\n(First 20 chars: `{fetched_token[:20]}...`)")
                                else:
                                    await prog_msg.edit_text("❌ **Error: Token not found in API response.**\nUsing placeholder token.")
                                    cptoken = "AUTO_FETCH_FAILED_USE_MANUAL_TOKEN_OR_HELPER"
                                    pwtoken = "AUTO_FETCH_FAILED_USE_MANUAL_TOKEN_OR_HELPER"
                                    cwtoken = 'eyJhbGc...cw_token...'
                            else:
                                error_msg = data.get("error", "Unknown error from your API")
                                await prog_msg.edit_text(f"❌ **API Error:** {error_msg}\nUsing placeholder token.")
                                cptoken = "AUTO_FETCH_FAILED_USE_MANUAL_TOKEN_OR_HELPER"
                                pwtoken = "AUTO_FETCH_FAILED_USE_MANUAL_TOKEN_OR_HELPER"
                                cwtoken = 'eyJhbGc...cw_token...'
                        else:
                            error_text = await resp.text()
                            await prog_msg.edit_text(f"❌ **Failed to fetch token from your API. Status: {resp.status}**\nResponse: {error_text[:200]}\nUsing placeholder token.")
                            cptoken = "AUTO_FETCH_FAILED_USE_MANUAL_TOKEN_OR_HELPER"
                            pwtoken = "AUTO_FETCH_FAILED_USE_MANUAL_TOKEN_OR_HELPER"
                            cwtoken = 'eyJhbGc...cw_token...'
            except aiohttp.ClientConnectorError as conn_err:
                await prog_msg.edit_text(f"❌ **Network Error connecting to your API:** {conn_err}\nUsing placeholder token.")
                cptoken = "AUTO_FETCH_FAILED_NETWORK_ERROR"
                pwtoken = "AUTO_FETCH_FAILED_NETWORK_ERROR"
                cwtoken = 'eyJhbGc...cw_token...'
            except asyncio.TimeoutError:
                await prog_msg.edit_text(f"❌ **Timeout Error calling your API.**\nUsing placeholder token.")
                cptoken = "AUTO_FETCH_FAILED_TIMEOUT"
                pwtoken = "AUTO_FETCH_FAILED_TIMEOUT"
                cwtoken = 'eyJhbGc...cw_token...'
            except Exception as e:
                await prog_msg.edit_text(f"❌ **Exception while fetching token from your API:** {e}\nUsing placeholder token.")
                cptoken = "AUTO_FETCH_FAILED_EXCEPTION"
                pwtoken = "AUTO_FETCH_FAILED_EXCEPTION"
                cwtoken = 'eyJhbGc...cw_token...'
        else:
            cwtoken = raw_text4
            cptoken = raw_text4
    ...
    if raw_text6.startswith("http://") or raw_text6.startswith("https://"):
        ...
        thumb = sanitize_filename(raw_text6) if raw_text6 != '/d' else raw_text6
    await editable.delete()
    try:
        if raw_text == "1":
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            name = sanitize_filename(f'{name1[:60]}')
            ...
            if "acecwply" in url:
                ...
                prog_msg_drm = await m.reply_text("**🔄 Processing DRM URL with your deployed API...**")
                try:
                    async with aiohttp.ClientSession() as session:
                        api_drm_url = f"{YOUR_DEPLOYED_API_BASE_URL}/v1/api"
                        params = {"url": url}
                        async with session.get(api_drm_url, params=params) as resp:
                            if resp.status == 200:
                                data = await resp.json()
                                if data.get("success") and data.get("url"):
                                    # Direct assignment instead of calling helper
                                    mpd = data["url"]  # Use the signed MPD URL directly
                                    url = mpd
                                    keys = []  # Keys are handled by downloader
                                    keys_string = ""  # No additional keys needed
                                    await prog_msg_drm.edit_text("**✅ DRM URL processed successfully by your API.**")
                                else:
                                    error_msg = data.get("error", "Unknown error from your API")
                                    await prog_msg_drm.edit_text(f"❌ **Your API Error (cpvod):** {error_msg}")
                                    continue
                            else:
                                error_text = await resp.text()
                                await prog_msg_drm.edit_text(f"❌ **Failed calling your API (cpvod). Status: {resp.status}**\n```\n{error_text[:200]}\n```")
                                continue
                except aiohttp.ClientConnectorError as conn_err:
                    await prog_msg_drm.edit_text(f"❌ **Network Error connecting to your API (cpvod):** {conn_err}")
                    continue
                except asyncio.TimeoutError:
                    await prog_msg_drm.edit_text(f"❌ **Timeout Error calling your API (cpvod).**")
                    continue
                except Exception as e:
                    await prog_msg_drm.edit_text(f"❌ **Exception while calling your API (cpvod):** {e}")
                    continue
            elif "classplusapp.com/drm/" in url:
                prog_msg_drm2 = await m.reply_text("**🔄 Processing DRM URL with your deployed API...**")
                try:
                    async with aiohttp.ClientSession() as session:
                        api_drm_url = f"{YOUR_DEPLOYED_API_BASE_URL}/v1/api"
                        params = {"url": url}
                        async with session.get(api_drm_url, params=params) as resp:
                            if resp.status == 200:
                                ...
                                if data.get("success") and data.get("url"):
                                    ...
                                    keys = []  # Keys are handled by downloader
                                    keys_string = ""  # No additional keys needed
                                    await prog_msg_drm2.edit_text("**✅ DRM URL processed successfully by your API.**")
                                else:
                                    error_msg = data.get("error", "Unknown error from your API")
                                    await prog_msg_drm2.edit_text(f"❌ **Your API Error (drm):** {error_msg}")
                                    continue
                            else:
                                error_text = await resp.text()
                                await prog_msg_drm2.edit_text(f"❌ **Failed calling your API (drm). Status: {resp.status}**\n```\n{error_text[:200]}\n```")
                                continue
                except aiohttp.ClientConnectorError as conn_err:
                    await prog_msg_drm2.edit_text(f"❌ **Network Error connecting to your API (drm):** {conn_err}")
                    continue
                except asyncio.TimeoutError:
                    await prog_msg_drm2.edit_text(f"❌ **Timeout Error calling your API (drm).**")
                    continue
                except Exception as e:
                    await prog_msg_drm2.edit_text(f"❌ **Exception while calling your API (drm):** {e}")
                    continue
            ...
            try:
                ...
                if "drive" in url:
                    ...
                    res_file = await helper.download_and_decrypt_video(url, cmd, sanitize_filename(name), appxkey)  
                    filename = res_file  
                    await prog.delete(True)  
                    await helper.send_vid(bot, m, cc, filename, thumb, sanitize_filename(name), prog)  
                    count += 1  
                    ...
                    Show = f"<blockquote>🚀𝐏𝐫𝐨𝐠𝐫𝐞𝐬𝐬 » {progress:.2f}%</blockquote>\n┃\n" \
                           ...
                           f"╰━✦𝐁𝐨𝐭 𝐌𝐚𝐝𝐞 𝐁𝐲 ✦ {CREDIT}"
                    prog = await m.reply_text(Show, disable_web_page_preview=True)
                    res_file = await helper.decrypt_and_merge_video(mpd, keys_string, path, sanitize_filename(name), raw_text2)
                    filename = res_file
                    await prog.delete(True)
                    await helper.send_vid(bot, m, cc, filename, thumb, sanitize_filename(name), prog)
                    count += 1
                    ...
                    Show = f"<blockquote>🚀𝐏𝐫𝐨𝐠𝐫𝐞𝐬𝐬 » {progress:.2f}%</blockquote>\n┃\n" \
                           ...
                           f"╰━✦𝐁𝐨𝐭 𝐌𝐚𝐝𝐞 𝐁𝐲 ✦ {CREDIT}"
                    prog = await m.reply_text(Show, disable_web_page_preview=True)
                    res_file = await helper.download_video(url, cmd, sanitize_filename(name))
                    filename = res_file
                    await prog.delete(True)
                    await helper.send_vid(bot, m, cc, filename, thumb, sanitize_filename(name), prog)
                    count += 1
    ...
    try:
            ...
            try:
                ...
                if "drive" in url:
                    ...
                    await m.reply_text(f"⚠️𝐃𝐨𝐰ɴʟᴏᴀ𝐝ɪɧɢ 𝐈ɴᴛᴇʀʀᴜᴘᴛᴇᴅ\n\n🔗𝐋𝐢𝐧𝐤 » `{link}`\n\n__**⚠️Failed Reason »**__\n<blockquote>{str(e)}</blockquote>")
                    pass
    except Exception as e:
        await m.reply_text(str(e))
